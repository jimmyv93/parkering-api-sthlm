// ----------------------------------------------------------------
// BYTA UT: Ersätt med din GitHub rådata-URL efter att du skapat
...
export default async function handler(req, res) {
  ...
  // ----------------------------------------------------------------
  // Hämta garagedata från GitHub (statisk JSON, snabb)
  // ----------------------------------------------------------------
  ...
  if (GARAGE_KEYWORDS.some(kw => lastUserMessage.includes(kw))) {
    try {
      const res2 = await fetch(GARAGE_DATA_URL, {
        signal: AbortSignal.timeout(3000),
        headers: { 'Cache-Control': 'no-cache' },
      });
      const anlaggningar = await res2.json();

      if (anlaggningar && anlaggningar.length > 0) {
        ...
        garageContext = `--- Stockholm Parkering — anläggningar (uppdateras var 10:e minut) ---
${sammanfattning}`;
      }
    } catch (err) {
      console.error('Kunde inte hämta garagedata från GitHub:', err.message);
    }
  }
...
När du har data från Stockholm Parkering, använd den för att ge konkreta svar
